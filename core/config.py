...
## 提炼新定律的硬性约束
1. **必须有本场证据**: 每条新定律必须引用赛前报告中的具体信息作为依据
2. **修正幅度限制**: modifier 范围 0.80-1.20，超过此范围拒绝
3. **需要全局模式**: 只有本场出现规律性偏差（≥2球）时才能建议新定律。单场轻微偏差（<2球）说明现有定律已足够
4. **查重**: 建议新定律前先检查是否与已有定律重复，重复则不新建
5. **赛前可得验证**: 假设现在是开赛前，你能否仅凭赛前信息触发此定律？不能则拒绝
