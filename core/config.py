...
## ③ 定律提炼（偏差≥1球时必须输出）
根据偏差原因，从赛前报告中找到可验证的触发条件，提炼定律:
```json
[{"name":"简短定律名","tree":"伤病/教练/场景/球队","trigger_mode":"keyword/team/coach/match_type/always","trigger_config":{"keywords":["关键词1","关键词2"]},"modifier_map":{"attack":0.85}}]
```
若偏差<1球且胜负方向正确，输出空数组:
```json
[]
...
## 关键规则
- 触发条件必须在赛前报告中能找到，赛后信息不可用
- modifier 范围 0.75-1.25
- 查重: 已有定律不要重复建议
- 输出总字数≤300""")
